...
public class BooleanPrimitiveSize {
	...
	public static void main(String[] args) {
	     ...
	     * platform independent. JVM  makes Java platform-independent by executing bytecodes which are generated by java compiler in
	     * in the form of .class files.
	     * 
	     * So java compiler creates a bytecode. This bytecode will be same on all platforms. The the actual magic happens after bytecode.
	     * This bytecode is further converted to machine language by JVM. JVM creates this machine level code from bytecode as per the
	     * machine and thats how underlying platform is irrelevant to java.
	     * 
	     * Similar to java-gc, JIT is also a component of JVM. JIT is responsible for creating the machine level code from bytecode.
	     * Various components of JVM have various responsibilities for example gc is for memory management in JVM, JIT is to improve
	     * performance (execution time) of code in JVM.
	     * 
	     * How JIT improves performance of JVM ?
	     * => Since the execution of bytecode is slower than the execution of machine language code, because JVM first needs to
	     * 		translate bytecode into machine language code so JIT provides a solution to this by converting only the bytecode that
	     * 		changes frequently (aka hot-code). JIT also offers caching of compiled code for frequent executions.
	     * 
	     * 
	     * Reference: Read more: https://www.java67.com/2013/02/difference-between-jit-and-jvm-in-java.html#ixzz6Q0wAlfRn
	     */
	}
